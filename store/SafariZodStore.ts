...
const hotelSchema = z.object({
  ...
  images: z
    .array(z.instanceof(File, { error: "Each hotel image must be a valid file" }))
    .min(1, { error: "At least one hotel image is required" }),
});

const daySchema = z.object({
  day: z.number({ error : "Day must be a number" })
  ...
  images: z
    .array(z.instanceof(File, { error: "Each day image must be a valid file" }))
    .min(1, { error: "At least one day image is required" }),
  hotel: hotelSchema,
});
