...
const CostIncludedUpdate = () => {
  ...
  const { fields, remove, append } = useFieldArray({
    control,
    name: "cost_inclusive", // 👈 match schema
  });

  const handleAppend = () => {
    append(""); // since it's string[], append empty string
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          Add items that are <strong>included</strong> in the tour price
        </p>
      </div>
      ...
      {fields.length === 0 && (
        ...
        >
          ...
          <p className="text-green-600 text-sm">
            Click "Add Inclusion" to start listing what’s included
          </p>
        </motion.div>
      )}
          ...
          >
            <div className="space-y-3">
              ...
              <div className="relative">
                <input
                  {...register(`cost_inclusive.${index}`)} // 👈 directly string[]
                  placeholder="e.g., Gorilla permits (One per person)"
      ...
      <AnimatePresence>
        {errors.cost_inclusive && (
          ...
          >
            {errors.cost_inclusive?.message as string}
          </motion.p>
        )}
      </AnimatePresence>
...
export default CostIncludedUpdate;
