...
const TextAreaInput = ({ title, name, placeholder, rows = 5 }: TextAreaInputProp) => {
  const { register, formState: { errors } } = useFormContext()
  const hasError = errors[name]
  const errorMessage = hasError?.message as string | undefined
  ...
  return (
    <div className="relative w-full">
      ...
      <textarea
        ...
        rows={rows}
        className={`
          peer w-full px-3 pt-6 pb-2 rounded-lg border-2 transition-all duration-200 outline-none resize-none
